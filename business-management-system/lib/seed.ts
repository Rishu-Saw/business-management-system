...
export function buildSeed(): AppState {
  ...
  const employees: Employee[] = EMPLOYEES.map(
    ([name, position, department, role, joinedDaysAgo], i) => ({
      id: id("emp", i + 1),
      name,
      email: `${name.split(" ")[0].toLowerCase()}@technovasolutions.in`,
  ...
  const items: Item[] = [
    ...PRODUCTS.map(([name, category, price, cost, taxRate], i) => {
      ...
      return {
        ...
        name,
        sku: `TN-${category.slice(0, 3).toUpperCase()}-${pad(1001 + i, 4)}`,
        description: `${name} — supplied, configured and warranty-backed by TechNova Solutions.`,
        category,
        price,
        costPrice: cost,
        taxRate,
    ...
    ...SERVICES.map(([name, category, price, taxRate, duration], i) => ({
      ...
      name,
      sku: `TN-SVC-${pad(2001 + i, 4)}`,
      description: `${name} delivered by TechNova's certified consulting team.`,
      category,
      price,
  ...
  return {
    business: {
      ...
      email: "accounts@technovasolutions.in",
